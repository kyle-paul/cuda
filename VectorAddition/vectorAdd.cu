...
int main() {
    ...
    std::vector<int> da_cpu; da_cpu.reserve(N);
    std::vector<int> db_cpu; db_cpu.reserve(N);

    cudaMemcpy(da_cpu.data(), d_a, bytes, cudaMemcpyDeviceToHost);
    cudaMemcpy(db_cpu.data(), d_b, bytes, cudaMemcpyDeviceToHost);
